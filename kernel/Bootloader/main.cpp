...
extern "C" EFI_STATUS efi_main(EFI_HANDLE imgHandle, EFI_SYSTEM_TABLE* sysTable)
{
    ...
    for(UINT32 m = 0; m < EFIUtil::Graphics->Mode->MaxMode; m++) {
        ...
        if(info->VerticalResolution > resBestY && info->HorizontalResolution <= 1280 && info->VerticalResolution <= 720 && (info->PixelFormat == PixelBlueGreenRedReserved8BitPerColor || info->PixelFormat == PixelRedGreenBlueReserved8BitPerColor)) {
