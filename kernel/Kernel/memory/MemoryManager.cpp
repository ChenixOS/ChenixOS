...
namespace MemoryManager {
    ...
    int MmQueryMemory(const void* virt) {
        uint64 pml4Index = GET_PML4_INDEX((uint64)virt);
        ...
        return (pml1[pml1Index] != 0);
    }

    SYSCALL_DEFINE1(syscall_mm_query,void* ptr) {
        return MmQueryMemory(ptr);
    }
    ...
    void* UserOrKernelToPhysPtr(void* addr) {
        void* res = addr;
        if((unsigned long)addr < g_HighMemBase && IsUserPtr(addr)) {
            res = UserToKernelPtr(addr);
        }
        return KernelToPhysPtr(res);
    }
