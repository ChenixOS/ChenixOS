...
namespace VFS {
    ...
    void Directory::CopyTo(Directory** dir,Directory** to) {
        for(uint64 i = 0;i < (*dir)->numEntries;i++) {
            DirectoryEntry* outEntry;
            Directory::AddEntry(to,&outEntry);

            kmemcpy(outEntry->name,(*dir)->entries[i].name,sizeof(DirectoryEntry::name));
            outEntry->nodeID = (*dir)->entries[i].nodeID;
        }
    }
