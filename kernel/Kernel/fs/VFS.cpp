...
namespace VFS {
    ...
    int64 Open(const char* path, uint64 openMode, uint64& fileDesc,VFS::Node** outNode) {
        char cleanBuffer[255];
        char pathBuffer[255];
        ...
        // 复制出绝对路径保存备用
        kstrncpy(pathBuffer,cleanBuffer,254);
        ...
        // 复制路径到FD
        kstrncpy(desc->path,pathBuffer,255);
    ...
    int64 Open(const char* path, uint64 openMode, uint64& fileDesc) {
        return Open(path,openMode,fileDesc,nullptr);
    }
