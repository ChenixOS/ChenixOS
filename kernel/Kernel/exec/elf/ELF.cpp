...
bool ELFExecHandler::CheckAndPrepare(uint8* buffer, uint64 bufferSize, uint64 pml4Entry, IDT::Registers* regs, int argc, const char* const* argv,int envCount,const char* const* env)
{
    ...
    char** userEnv = new char*[envCount + 1];
    for(int i = 0;i < envCount;i++) {
        int l = kstrlen(env[i]) + 1;
        virtStack -= l;
        envSize += l;
        kmemcpy(virtStack, env[i], l);
        userEnv[i] = (char*)stackBase + 16 * 4096 - argSize - envSize;
    }

    for(int i = 0;i < envCount;i++) {
        virtStack -= sizeof(char*);
        envSize += sizeof(char*);
        *(char**)virtStack = userEnv[i];
    }
    virtStack -= sizeof(char*);
    envSize += sizeof(char*);
    *(char**)virtStack = nullptr;
    ...
    progInfo.env = (char**)(stackBase + 16 * 4096 - argSize - envSize);
    ...
    regs->rip = entryPoint;
    regs->userrsp = stackBase + 16 * 4096 - argSize - envSize - sizeof(ELFProgramInfo);
    regs->rdi = stackBase + 16 * 4096 - argSize - envSize - sizeof(ELFProgramInfo);
