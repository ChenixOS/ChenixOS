...
namespace PCI {
    ...
    void WriteConfigMask(const Device& dev, uint32 reg,uint16 off, uint16 on) {
        uint16 val = ReadConfigWord(dev,reg);
        val = (val & ~off) | on;
        WriteConfigWord(dev, reg, val);
    }

    void EnableBusMaster(const Device& dev) {
        barrier();
        WriteConfigMask(dev,PCI_COMMAND,0,PCI_COMMAND_MASTER);
    }

    void EnableMemoryMaster(const Device& dev) {
        barrier();
        WriteConfigMask(dev,PCI_COMMAND,0,PCI_COMMAND_MEMORY);
    }
