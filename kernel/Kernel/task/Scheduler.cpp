...
namespace Scheduler {
    ...
    static Atomic<uint64> g_PIDCounter = 1;
    ...
    static void SetupKillHandlerFromTick(ThreadInfo* tInfo) {
        if(tInfo->killHandlerRip != 0) {
            ...
            tInfo->registers.rdi = tInfo->killHandlerSignum;
        } else {
    ...
    SYSCALL_DEFINE4(syscall_thread_create, uint64 entry, uint64 stack, uint64 arg,uint64 arg1) {
        IDT::Registers regs;
        regs.rip = entry;
        regs.userrsp = stack;
        regs.rdi = arg;
    ...
    SYSCALL_DEFINE1(syscall_thread_detach, int64 tid) {
    ...
    SYSCALL_DEFINE1(syscall_thread_join, int64 tid) {
        int64 exitCode;
    ...
    SYSCALL_DEFINE1(syscall_thread_tryjoin, int64 tid) {
        int64 exitCode;
    ...
    int64 ThreadKillSig(int64 tid,int64 signum) {
        auto tInfo = g_CPUData.Get().currentThread;
        auto uid = tInfo->uid;
        ...
        killThread->killHandlerSignum = signum;
        g_GlobalThreadListLock.Unlock();
        return OK;
    }

    int64 ThreadKill(int64 tid) {
        return ThreadKillSig(tid,0);
    }

    SYSCALL_DEFINE2(syscall_thread_kill, int64 tid, int64 signum) {
        return ThreadKillSig(tid, signum);
    }
    ...
    void KillChildThreads(ThreadInfo* tInfo) {
        while(true) {
    ...
    SYSCALL_DEFINE1(syscall_thread_exit, uint64 code) {
        ThreadExit(code);
        return 1;
    }
    ...
    static void SetupKillHandlerFromSyscall(SyscallState* state, uint64 retVal) {
        ...
        if(tInfo->killHandlerRip != 0) {
            ...
            state->userrdi = tInfo->killHandlerSignum;
        } else {
            ThreadExit(1);
        }
    }
    ...
    SYSCALL_DEFINE0(syscall_getuid) {
        return GetCurrentThreadInfo()->uid;
    }
    SYSCALL_DEFINE0(syscall_getgid) {
        return GetCurrentThreadInfo()->gid;
    }

    SYSCALL_DEFINE0(syscall_getpid) {
        return GetCurrentThreadInfo()->pid;
    }
    SYSCALL_DEFINE0(syscall_getppid) {
        return GetCurrentThreadInfo()->ppid;
    }

    // Process

    void ProcessDoExit(uint64 code) {
        auto& cpuData = g_CPUData.Get();
        auto tInfo = cpuData.currentThread;

        // 获取主线程，并且杀掉所有子进程
        if(tInfo->mainThread != nullptr)
            tInfo = tInfo->mainThread;

        KillChildThreads(cpuData.currentThread);
        KillChildThreads(tInfo);
    ...
    // 查找出所有进程PID
    Vector<int64>* ListAllProcessId() {
        Vector<int64>* result = new Vector<int64>();

        g_GlobalThreadListLock.Spinlock();
        for(auto &t : g_GlobalThreadList) {
            if(t.mainThread == nullptr || t.pid == t.tid) {
    ...
    bool FindThreadInfoAndCopy(int64 tid,struct ThreadInfo* out) {
        bool result = false;

        g_GlobalThreadListLock.Spinlock();
        for(auto &t : g_GlobalThreadList) {
            if(t.tid == tid) {
                kmemcpy(out, (const void*)&t, sizeof(struct ThreadInfo));
                result = true;
                break;
            }
        }
    ...
    bool FindProcessMainThreadInfoAndCopy(int64 pid,struct ThreadInfo* out) {
        bool result = false;
        
        g_GlobalThreadListLock.Spinlock();
        for(auto &t : g_GlobalThreadList) {
            if(t.pid == pid || t.tid == pid) {
                struct ThreadInfo* src = (t.mainThread == nullptr) ? &t : t.mainThread;

                kmemcpy(out, (const void*) src, sizeof(struct ThreadInfo));
                result = true;
                break;
            }
        }
